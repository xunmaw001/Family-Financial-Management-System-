                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input id="idNumber" name="idNumber" class="form-control"
                                           onchange="idNumberChickValue(this)"  placeholder="身份证">
                                </div>
                                   <div class="form-group col-md-6">
                                       <label>家庭名称</label>
                                       <select id="jtTypesSelect" name="jtTypes" class="form-control">
                                       </select>
                                   </div>
...
<script>
    ...
    // 查询列表
        ...
        function jtTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jt_types", "GET", {}, (res) => {
                if(res.code == 0){
                    jtTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    <!-- 初始化当前表的下拉框 -->
        ...
        function initializationJttypesSelect(){
            var jtTypesSelect = document.getElementById('jtTypesSelect');
            if(jtTypesSelect != null && jtTypesOptions != null  && jtTypesOptions.length > 0 ){
                for (var i = 0; i < jtTypesOptions.length; i++) {
                    jtTypesSelect.add(new Option(jtTypesOptions[i].indexName,jtTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(jtTypesSelect != null && jtTypesOptions != null  && jtTypesOptions.length > 0 ) {
            for (var i = 0; i < jtTypesOptions.length; i++) {
                if (jtTypesOptions[i].codeIndex == ruleForm.jtTypes) {//下拉框value对比,如果一致就赋值汉字
                        $("#jtTypesSelect").attr('style', 'pointer-events: none;');
                        jtTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
